...
void BST::RevNodes(TNode* curNode)
{
	if(curNode != NULL)
	{
...
// the desired order.
  ...
  switch (order) {
    ...
    case REV_IN_ORDER: RevNodes(root);
    				break;
  }
  
}

int BST::HeightHelper(TNode* curNode)
{
	if(curNode == NULL)
	{
		return -1;
	}
	else if(HeightHelper(curNode->left) > HeightHelper(curNode->right))
	{
		return HeightHelper(curNode->left) + 1;
	}
	else
	{
		return HeightHelper(curNode->right) + 1;
	}
}
...
double BST::GetIHRatio()
{
	...
	double ratio = (double) GetHeight() / idealHeight;
	return ratio;
}
